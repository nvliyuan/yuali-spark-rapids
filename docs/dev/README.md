...
known, and use `versionNotes(...)` for compatibility caveats such as renamed,
deprecated, replaced, or removed configs. If version metadata is not recorded,
the generated docs show `Unknown` in the `Since Version` column.
